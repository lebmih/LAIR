...
contigs <- read.table("contigs_040720.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
# Analyzing only the naturally occurring inserts.
contigs <- contigs %>%
  dplyr::filter(ins.type.simple %in% c("VDJ", "J-CH1", "V-CH1") &
...
write.table(clusters, file = "clusters_040720.txt", quote = FALSE,
            sep = "\t", row.names = FALSE)
# Extracting only the clusters found in more than one donor
clusters <- clusters %>% dplyr::filter(donors.n > 1)
write.table(clusters, file = "hotspots_040720.txt", quote = FALSE,
            sep = "\t", row.names = FALSE)
clusters <- read.table("hotspots_040720.txt", header = TRUE, sep = "\t",
